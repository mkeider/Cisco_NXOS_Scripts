...
#################################################################################################
#Script connects to cisco_nxos device extracts bgp neighbor data, parses with textfsm, then      #
#outputs to .csv "nxos_bgp_neighbor.csv"                                                         #
#################################################################################################
...
for device in devices:
    ...
    try:
        ...
        print(local_host)
        output = connection.send_command("show ip bgp summary", use_textfsm=True)##textfsm integrated into Netmiko
...
with open('nxos_bgp_results.json', 'w') as results_file:
    json.dump(results, results_file, indent=2)


with open('nxos_bgp_summary.csv','w', newline='')as new_file:
   fieldnames = ['local_host','bgp_neigh','neigh_as','state_pfxrcd']
