...
#################################################################################################
#Script connects to cisco_nxos device extracts port-channel data, parses with textfsm, then     #
#outputs to .csv "nxos_show_port_channel.csv"                                                   #
#################################################################################################
...
for device in devices:
    ...
    try:
        ...
        print(local_host)
        output = connection.send_command("show port-channel summary", use_textfsm=True)##textfsm integrated into Netmiko

        for item in output:
            item.update({"local_host":local_host})#Insert Local Hostname into dictionary
            port_channel_results.append(item) #For Loop to append the OUTPUT results from connection.send_command
...
print(port_channel_results)

print(json.dumps(results, indent=2))
with open('nxos_port_channel_results.json', 'w') as results_file:
    json.dump(results, results_file, indent=2)


with open('nxos_show_port_channel.csv','w', newline='')as new_file:
   fieldnames = ['local_host','bundle_iface','bundle_proto','bundle_proto_state', 'bundle_status', 'phys_iface', 'phys_iface_status']
   csv_writer = csv.DictWriter(new_file, fieldnames=fieldnames, delimiter=',')

   csv_writer.writeheader()

   for line in (port_channel_results):
