...
#################################################################################################
#Script connects to cisco_nxos device extracts show vpc data, parses with textfsm, then         #
#outputs to .csv "nxos_show_vpc.csv"                                                            #
#################################################################################################
...
for device in devices:
    ...
    try:
        ...
        print(local_host)
        output = connection.send_command("show vpc", use_textfsm=True)##textfsm integrated into Netmiko

        for item in output:
            item.update({"local_host":local_host})#Insert Local Hostname into dictionary
            vpc_results.append(item) #For Loop to append the OUTPUT results from connection.send_command
...
with open('nxos_vpc_results.json', 'w') as results_file:
    json.dump(results, results_file, indent=2)


with open('nxos_show_vpc.csv','w', newline='')as new_file:
   fieldnames = ['local_host','id','port','status']
   csv_writer = csv.DictWriter(new_file, fieldnames=fieldnames, delimiter=',')

   csv_writer.writeheader()

   for line in (vpc_results):
