...
#################################################################################################
#Script connects to cisco_nxos device extracts show feature, parses with textfsm, then          #
#outputs to .csv "nxos_feature.csv"                                                             #
#################################################################################################
...
for device in devices:
    ...
    try:
        ...
        print(local_host)
        output = connection.send_command("show feature", use_textfsm=True)##textfsm integrated into Netmiko

        for item in output:
            item.update({"local_host":local_host})#Insert Local Hostname into dictionary
            feature_results.append(item) #For Loop to append the OUTPUT results from connection.send_command
...
with open('nxos_feature_results.json', 'w') as results_file:
    json.dump(results, results_file, indent=2)


with open('nxos_feature.csv','w', newline='')as new_file:
   fieldnames = ['local_host','name','instance','state']
   csv_writer = csv.DictWriter(new_file, fieldnames=fieldnames, delimiter=',')

   csv_writer.writeheader()

   for line in (feature_results):
